...
export default class YmlToJson {
	constructor(editor){
		...
		if(o != undefined){
			var str=this.createHtml(o,editor);
	...
	replaceOrigin(data){
		...
		if(data.AK) data.AK = data.AK.replace(/\\/gi,"");
		if(data.KO) data.KO = data.KO.replace(/\\/gi,"");
		if(data.EN) data.EN = data.EN.replace(/\\/gi,"");
		if(data.IMG) data.IMG = data.IMG.replace(/\\/gi,"");
		return data;
	}
	insertTag(data,editor){
		...
		if(data.OR){
			...
			}else if(data.tagname.indexOf("table")>-1){
				var tempArray = data.OR.split('\n');
				var textTemp = '';
                for(var i=0; i<tempArray.length; i++) {
                    var textTemp2 = '';
                    textTemp2 = tempArray[i].replace(/\t\t\t\t/g, '|');
                    textTemp2 = textTemp2.replace(/\t/g, '|');
                    textTemp += textTemp2+'|'+'\n';
                    if(i == 0) {
                        var cnt = (textTemp.match(/\|/g) || []).length;
                        var sort = '';
                        for(var j=0; j<cnt; j++) {
                            sort += '---|';
                        }
		...
		if(data.AK){
			if(data.tagname.indexOf("h")>-1){
				var content = "<"+data.tagname+" class='chinese sec_node or'>";
				content += '<a href="javascript:;" role="" title="표제어 펼치기/접기" style="cursor: default;">';
				content += '<span class="sec_title" id="contentOr">'+data.AK+'</span>'
				content += '</a>';
				content += "</"+data.tagname+">";
				data.AK = content;
			}else if(data.tagname.indexOf("p")>-1){
				data.AK = "<dd class='chinese or sec_node' id='contentOr'>"+data.AK+"</dd>";
			}else if(data.tagname.indexOf("table")>-1){
				var tempArray = data.AK.split('\n');
				var textTemp = '';
                for(var i=0; i<tempArray.length; i++) {
                    var textTemp2 = '';
                    textTemp2 = tempArray[i].replace(/\t\t\t\t/g, '|');
		...
		if(data.IMG){
			var imgPath=editor.getPath();
			var idx=imgPath.indexOf("contents");
			imgPath=imgPath.substr(0,idx);
			imgPath=imgPath+"contents\\images\\"
			var src=data.IMG.replace(/(<)(\w+\.\w+)(>)/,"$2");
			var temp = "<img src="+imgPath+src+">";
			data.IMG=temp;
		}
		return data;
	}
	writeFile(filePath,data){
	  ...
	  var oldPath =atom.packages.getPackageDirPaths()+"\\05-kiom-makeHtml\\html";
